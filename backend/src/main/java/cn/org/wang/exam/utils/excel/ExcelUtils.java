package cn.org.wang.exam.utils.excel;
...
public class ExcelUtils {
        ...
        if (uniqueBuilder.length() > 0) {
            if (uniqueMap.containsValue(uniqueBuilder.toString())) {
                for (Map.Entry<Integer, String> entry : uniqueMap.entrySet()) {
                    if (entry.getValue().equals(uniqueBuilder.toString())) {
                        errMsgList.add(String.format("数据唯一性校验失败,(%s)与第%s行重复)", uniqueBuilder, entry.getKey()));
                    }
                }
            } else {
    ...
    private static Workbook getWorkbook(MultipartFile mFile, File file) throws IOException {
        ...
        if (mFile != null) {
            ...
            fileName = (file != null ? file.getName().toLowerCase() : "");
        }
    ...
    private static JSONArray readSheet(Sheet sheet) {
        ...
        if (rowStart == rowEnd) {
            ...
            // 添加行号
            obj.put(ROW_NUM, 1);
            for (java.util.Map.Entry<Integer, String> entry : keyMap.entrySet()) {
    ...
    private static void write(HttpServletResponse response, SXSSFWorkbook book, String fileName) throws IOException {
        // 清除响应中可能存在的JSON格式内容（防止被拦截器修改）
        response.reset();
        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setCharacterEncoding("utf-8");

        // 文件名处理：移除特殊字符，使用RFC 5987编码支持中文
        String safeName = fileName.replaceAll("[\\\\/:*?\"<>|]", "_");
        String encodedName = URLEncoder.encode(safeName, "UTF-8").replaceAll("\\+", "%20");
